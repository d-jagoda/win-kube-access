...
while ($true) {
    ...
    try {
        Write-Output "[$(Get-Date)] Attempting to connect to $pipeName"
        $dockerPipe.Connect()
        Write-Output "[$(Get-Date)] Connect to $pipeName. Docker engine is running"
        ...
        Write-Output "[$(Get-Date)] Disconnected from $pipeName. Assuming Docker engine was shutdown"
    } finally {
        $dockerPipe.Dispose()
    }
}